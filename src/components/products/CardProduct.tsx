import { useState } from "react";
import { FiPlus } from "react-icons/fi";
import { Link } from "react-router-dom";
import { Variant } from "../../interfaces";

interface Props {
  img: string;
  name: string;
  price: number;
  slug: string;
  colors: { name: string; color: string }[];
  variants: Variant[];
}

export const CardProduct = ({
  img,
  name,
  price,
  slug,
  colors,
  variants,
}: Props) => {
  // Asegurarse de que haya al menos un color antes de setear
  const [activeColor, setActiveColor] = useState(colors[0]);

  const selectedVariant = variants.find(
    (variant) => variant.color?.hex_code === activeColor.color
  );

  const stock = selectedVariant?.stock ?? 0;

  return (
    <div className="flex flex-col gap-6 relative">
      <Link
        to={`/producto/${slug}`}
        className="flex relative group overflow-hidden"
      >
        <div className="flex h-[350px] w-full items-center justify-center py-2 lg:h-[250px]">
          <img src={img} alt={name} className="object-contain h-full w-full" />
        </div>

        <button className="bg-white border border-slate-200 absolute w-full bottom-0 py-3 rounded-3xl flex items-center justify-center gap-1 text-sm font-medium hover:bg-stone-100 translate-y-[100%] transition-all duration-300 group-hover:translate-y-0">
          <FiPlus />
          Añadir
        </button>
      </Link>

      <div className="flex flex-col gap-1 items-center">
        <p className="text-[15px] font-medium">{name}</p>
        <p className="text-[15px] font-medium">{price}</p>

        <div className="flex gap-3">
          {colors.map((color) => (
            <span
              key={color.color}
              className={`grid place-items-center w-5 h-5 rounded-full cursor-pointer ${
                activeColor.color === color.color ? "ring-2 ring-black" : ""
              }`}
              onClick={() => setActiveColor(color)}
            >
              <span
                className="w-[14px] h-[14px] rounded-full"
                style={{ backgroundColor: color.color }}
              />
            </span>
          ))}
        </div>
      </div>

      {stock === 0 && (
        <div className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded">
          Agotado
        </div>
      )}
    </div>
  );
};
